...
2. Is the type (taxonomic distinction and/or life stage) of parasite important in determining whether parasitism is associated with decreased body condition?
...
#### JT Load datasets 
```{r JT datasets, echo=FALSE, eval=FALSE}
...
```{r p.vig a/l stage, echo=FALSE, eval=FALSE}
#### 
...
P.vigilax.data.smi <- P.vigilax.data %>%
  ...
  mutate(avg_weight = ave(weight_g, FUN = mean)) %>%
  janitor::clean_names()
...
P.vigilax.data.smi <- P.vigilax.data.smi %>%
  mutate(smi_pimvig = weight_g*(pimvig_avg_w / standard_length_mm) ^ b_smi) %>%
  janitor::clean_names()

I.punctatus.data.smi <- I.punctatus.data.smi %>%
  mutate(smi_ictpun = weight_g*(ictpun_avg_w / standard_length_mm) ^ b_smi) %>%
  janitor::clean_names()

N.atherinoides.data.smi <- N.atherinoides.data.smi %>%
  mutate(smi_notath = weight_g*(notath_avg_w / standard_length_mm) ^ b_smi) %>%
  janitor::clean_names()

H.nuchalis.data.smi <- H.nuchalis.data.smi %>%
  mutate(smi_hybnuc = weight_g*(hybnuc_avg_w / standard_length_mm) ^ b_smi) %>%
  janitor::clean_names()

P.vigil.data.smi <- P.vigil.data.smi %>%
  mutate(smi_pervig = weight_g*(pervig_avg_w / standard_length_mm) ^ b_smi) %>%
  janitor::clean_names()

C.velifer.data.smi <- C.velifer.data.smi %>%
  mutate(smi_carvel = weight_g*(carvel_avg_w / standard_length_mm) ^ b_smi) %>%
  janitor::clean_names()

#G.affinis.data.smi <- G.affinis.data.smi %>%
 # mutate(smi_gamaff = weight_g*(gamaff_avg_w / standard_length_mm) ^ b_smi) %>%
 # janitor::clean_names()
```
...
summary(smi_pervig_lm <- lm(smi_pervig ~ psite_count + ci, data = P.vigil.data.smi))
summary(smi_carvel_lm <- lm(smi_carvel ~ psite_count + ci, data = C.velifer.data.smi))
#summary(smi_gamaff_lm <- lm(body_index_gamaff ~ psite_count + ci, data = G.affinis.data.smi))
```
...
#### using smi
# Generate predictions
mydf2 <- ggpredict(smi_pimvig_lm, terms = c("ci"))
  ...
  mutate(
    life_stage = case_when(
      ...
      psite_spp_x %in% c("META.HS", "META.UNK", "MYX.GO", "MYX.GEOM",
                         ...
                         "TREM.SK", "TREM.CHONK", "TREM.ENDOUS", "TREM.CL", "TREM.ISPOT") ~ "larval",
