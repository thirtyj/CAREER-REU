...
```{r JT datasets, echo=TRUE}
P.vigilax.data <- read.csv("C:/Users/thirt/Desktop/SuPeR Parasites/data/processed/Percina_vigil_processed_human_readable_UPDATED_2024.08.01.csv") %>% janitor::clean_names() %>%
  mutate(weight_mg = as.numeric(weight_mg)) %>% 
  mutate(weight_g = (weight_mg/1000))
...
H.nuchalis.data <- read_csv("C:/Users/thirt/Desktop/SuPeR Parasites/data/processed/Hybognathus_nuchalis_processed_human_readable_UPDATED_2024.08.01.csv") %>%
  janitor::clean_names() %>%
  mutate(weight_mg = as.numeric(weight_mg)) %>% 
  mutate(weight_g = (weight_mg/1000))
```
